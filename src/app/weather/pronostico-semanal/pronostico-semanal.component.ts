...
export class PronosticoSemanalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.service.cityFound) {
      
      this.service.getFiveDaysForecast().subscribe((response: any) => {

        this.allDaysForecast = response.list;        
        
        this.allDaysForecast.filter(forecast => {
        ...
        console.log(this.firstDayForecast.length);
        console.log(this.firstDayForecast.reduce((max, forecast) => forecast.main.temp > max ? forecast.main.temp : max ,0));
